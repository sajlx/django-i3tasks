...
- **`django_i3tasks/maintenance.py`**: `clean_old_task_executions()` / `old_task_executions()` — prune `TaskExecution` older than `I3TASKS.autoclean_older_than` (a `timedelta`; cascades to tries/results). Supports `batch_size` for chunked deletes (safe first purge of a big table). Three triggers: the `i3tasks_clean` management command (`--days`, `--batch-size`, `--dry-run`), the built-in `django_i3tasks.tasks.autoclean_task` (a `@TaskDecorator` task schedulable via `I3TASKS.schedules`), or calling the helper directly. `created_at` is indexed by migration `0007` (`CREATE INDEX CONCURRENTLY` on Postgres). The `0006` uuid migration also honours the setting (pre-prunes old rows) and is backend-conditional (Postgres `gen_random_uuid()` single UPDATE, else per-row loop).
